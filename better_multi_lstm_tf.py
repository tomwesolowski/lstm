...
def build_model():
  ...
  model.states = tf.reshape(model.states, [-1, state_size])
  ...
  model.predictions = tf.reshape(tf.nn.softmax(model.logits), [-1, series_size, num_classes])
  model.predictions = tf.transpose(model.predictions, perm=[1, 0, 2])

  model.loss = tf.nn.sparse_softmax_cross_entropy_with_logits(
      logits=model.logits, labels=tf.reshape(model.y_placeholder, [-1]))
